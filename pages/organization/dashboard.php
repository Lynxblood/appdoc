...
<?php
  require '../../config/dbcon.php';
   ...
   // Query for summary cards
    $draft_count_query = $conn->prepare("SELECT COUNT(*) FROM documents WHERE organization_id = ? AND user_id = ? AND status = 'draft'");
    $draft_count_query->bind_param("ii", $organization_id, $user_id);
    $draft_count_query->execute();
    $draft_count = $draft_count_query->get_result()->fetch_row()[0];
    $draft_count_query->close();

    $submitted_count_query = $conn->prepare("SELECT COUNT(*) FROM documents WHERE organization_id = ? AND user_id = ? AND status = 'submitted'");
    $submitted_count_query->bind_param("ii", $organization_id, $user_id);
    $submitted_count_query->execute();
    $submitted_count = $submitted_count_query->get_result()->fetch_row()[0];
    $submitted_count_query->close();

    $approved_count_query = $conn->prepare("SELECT COUNT(*) FROM documents WHERE organization_id = ? AND user_id = ? AND status = 'approved' AND YEAR(created_at) = YEAR(CURDATE())");
    $approved_count_query->bind_param("ii", $organization_id, $user_id);
    $approved_count_query->execute();
    $approved_count = $approved_count_query->get_result()->fetch_row()[0];
    $approved_count_query->close();

    // Query for Upcoming Events
    $upcoming_events_query = $conn->prepare("SELECT COUNT(*) FROM events WHERE organization_id = ? AND start_date >= CURDATE()");
    $upcoming_events_query->bind_param("i", $organization_id);
    $upcoming_events_query->execute();
    $upcoming_events_count = $upcoming_events_query->get_result()->fetch_row()[0];
    $upcoming_events_query->close();

    // Query for Recent Documents with Status
    $recent_documents_query = $conn->prepare("SELECT document_id, document_type, pdf_filename, status, created_at FROM documents WHERE organization_id = ? AND user_id = ? ORDER BY created_at DESC LIMIT 5");
    $recent_documents_query->bind_param("ii", $organization_id, $user_id);
    $recent_documents_query->execute();
    $recent_documents_result = $recent_documents_query->get_result();

    // Query for Upcoming Events List
    $upcoming_events_list_query = $conn->prepare("SELECT title, start_date, end_date, total_expenses FROM events WHERE organization_id = ? AND start_date >= CURDATE() ORDER BY start_date ASC LIMIT 5");
    $upcoming_events_list_query->bind_param("i", $organization_id);
    $upcoming_events_list_query->execute();
    $upcoming_events_list_result = $upcoming_events_list_query->get_result();

    // Query for Notifications
    $notifications_query = $conn->prepare("SELECT
        n.notification_id,
        n.message,
        n.is_read,
        n.created_at,
    ...
    FROM
        notifications n
    JOIN
        documents d ON n.document_id = d.document_id
    LEFT JOIN
        comments c ON d.document_id = c.document_id
    LEFT JOIN
            ...
            <div class="col-md-8 pt-4">
                <div class="row">
                    ...
                    <div class="col-12 mb-3">
                        <div class="card cardeve shadow" style="min-height: 250px;">
                            <div class="card-body">
                                <h4 class="card-title titlesummary text-center">Recent Updates</h4>
                                <hr class="m-0">
                                    ...
                                    <div class="list-group list-group-flush mt-3">
                                        <?php while ($notification = $notifications_result->fetch_assoc()): ?>
                                            <div class="list-group-item list-group-item-action d-flex justify-content-between align-items-center">
                                                <div>
                                                    <small class="text-muted"><?= htmlspecialchars($notification['first_name'] . ' ' . $notification['last_name']); ?></small>
                                                    <p class="mb-1"><strong>Document:</strong> <?= htmlspecialchars($notification['pdf_filename']); ?></p>
                                                    <p class="mb-0"><strong>Status:</strong> <span class="badge bg-<?= ($notification['status'] == 'pending') ? 'warning' : 'danger'; ?>"><?= htmlspecialchars($notification['status']); ?></span></p>
                                                    <?php if (!empty($notification['comment_text'])): ?>
                                                        <small class="text-secondary">"<?= htmlspecialchars($notification['comment_text']); ?>"</small>
                                                    <?php endif; ?>
                                                </div>
                                                <i class="fas fa-bell text-<?= ($notification['status'] == 'pending') ? 'warning' : 'danger'; ?>"></i>
                                            </div>
                                        <?php endwhile; ?>
                                    </div>
                                <?php else: ?>
            ...
            <div class="col-md-4">
                <div class="card eventcard shadow" style="height: 100%;">
                    <div class="card-body">
                        ...
                        <hr class="m-0">
                        <div class="list-group list-group-flush mt-3">
                            <?php if ($upcoming_events_list_result->num_rows > 0): ?>
                                <?php while ($event = $upcoming_events_list_result->fetch_assoc()): ?>
                                    <div class="list-group-item list-group-item-action d-flex flex-column align-items-start">
                                        <div class="d-flex w-100 justify-content-between">
                                            ...
                                            <small class="text-muted"><?= htmlspecialchars(formatDateTime(($event['start_date']))); ?></small>
                                        </div>
                                        <small class="text-muted">Budget: P<?= htmlspecialchars(number_format($event['total_expenses'], 2)); ?></small>
                                    </div>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <p class="text-center text-muted mt-5">No upcoming events.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <div class="container-fluid">
        <div class="container-fluid rounded-3 border border-secondary-subtle p-3 my-3 overflow-x-scroll">
            ...
            <table id="myTable" class="table table-striped text-start">
                ...
                <tbody>
                    <?php
                    if ($recent_documents_result->num_rows > 0) {
                        while ($row = $recent_documents_result->fetch_assoc()) {
                            echo "<tr>";
