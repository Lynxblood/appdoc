...
if ($id > 0 && $status !== '' && $user_id) {
    ...
    try {
        ...
        //     // at the location of the [ADVISER_SIGNATURE] or [osas_SIGNATURE] tag.
        ...
        // 3. Find the user ID of the osas
        $sql_osas = "SELECT user_id FROM users WHERE user_role = 'osas'";
        $stmt_osas = $conn->prepare($sql_osas);
        $stmt_osas->execute();
        $result_osas = $stmt_osas->get_result();
        $osas = $result_osas->fetch_assoc();
        $stmt_osas->close();

        if ($osas) {
            $osas_user_id = $osas['user_id'];
