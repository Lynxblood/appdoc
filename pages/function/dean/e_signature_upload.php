...
// Check if the user is logged in as an dean
if (empty($_SESSION['user_role']) || $_SESSION['user_role'] !== "dean") {
...
if (isset($_FILES['signatureImage']) && $_FILES['signatureImage']['error'] === UPLOAD_ERR_OK) {
    ...
    if (move_uploaded_file($file_tmp_path, $upload_path)) {
        ...
        if ($stmt->execute()) {
            echo "<script>alert('E-signature uploaded successfully!'); window.location.href='../../dean_pages/dashboard.php';</script>";
        } else {
