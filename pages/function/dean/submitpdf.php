...
if ($id > 0 && $status !== '' && $user_id) {
    ...
    try {
        ...
        //     // at the location of the [ADVISER_SIGNATURE] or [fssc_SIGNATURE] tag.
        ...
        // 3. Find the user ID of the fssc
        $sql_fssc = "SELECT user_id FROM users WHERE user_role = 'fssc'";
        $stmt_fssc = $conn->prepare($sql_fssc);
        $stmt_fssc->execute();
        $result_fssc = $stmt_fssc->get_result();
        $fssc = $result_fssc->fetch_assoc();
        $stmt_fssc->close();

        if ($fssc) {
            $fssc_user_id = $fssc['user_id'];
